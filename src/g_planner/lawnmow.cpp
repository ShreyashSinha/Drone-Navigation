...
vector<vector<double> > find_lawnmow_waypoint(float width_cap, float len_cap, vector<pair<double,pair<double,double>>> surface,float h_dist,
    ...
    if (const_cord == 'z' and ascending and !mirror){
        ...
        // cout<<curr_dX<<" "<<curr_dY<<" "<<curr_dZ<<endl;

        // cout<<diagonal_points[0][0]<<" "<<diagonal_points[0][1]<<endl;
        // cout<<diagonal_points[1][0]<<" "<<diagonal_points[1][1]<<endl;
...
vector<vector<double> > find_camera_waypoint(float width_cap, float len_cap, vector<pair<double,pair<double,double>>> surface,float h_dist,
                                                float v_dist, bool ascending, float drone_dist, bool& mirror, float& path_length){
...
vector<vector<double> > find_preliminary_waypoints(float width_cap, float len_cap, vector<pair<double,pair<double,double>>> surface,float h_dist,
                                                float v_dist, float drone_dist, float& path_length){
        int flag = 1;
        path_length = len_cap/2;
        vector<double> tmp;
        vector<vector<double>> point;

        float curr_dY = surface[0].second.first;
        float curr_dX = surface[0].first;
        float curr_dZ = surface[0].second.second + drone_dist;
        for (;curr_dX < surface[1].first; curr_dX += 2.5){
            tmp.push_back(curr_dX);
            tmp.push_back(curr_dY);
            tmp.push_back(curr_dZ);
            point.push_back(tmp);
            ...
            if (flag){
                for (;curr_dY < surface[1].second.first; curr_dY += len_cap - h_dist){
                }
                tmp.push_back(curr_dX);
                tmp.push_back(curr_dY);
                tmp.push_back(curr_dZ);
                ...
                for (;curr_dY > surface[0].second.first; curr_dY -= len_cap - h_dist){
                }
                tmp.push_back(curr_dX);
                tmp.push_back(curr_dY);
                tmp.push_back(curr_dZ);
